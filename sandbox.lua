...
function NUM(n,s) 
  return {is=NUM, at=n,txt=s, _has={}, ok=false,
...
function data(data1,row)
  if   data1.cols 
  then push(data1.rows, cols(data1.cols, row)) 
...
--------- --------- --------- --------- --------- --------- -----
local function egs(    d,r1,r2,t)
  d=DATA(the.file)
  oo(stats(d)) 
  t={}
  for i=1,50 do 
     r1,r2 = l.rand.any(d.rows),  l.rand.any(d.rows) 
     push(t, m.rnd(minkowski(d, r1, r2),2)) end 
  oo(sorted(t)) end 
